...
#'
#' This function extracts from each spatial interaction model of the collection a
#' vector representation derived from its flow matrix (see details). This vector
#' is then used to compute distances between the models.
#'
#' The vector representation is selected using the `flows` parameters. Possible
#' values are
#'
#' -  `"full"` (default value): the representation is obtained by considering the
#'   matrix of [flows()] as a vector (with the standard [as.vector()] function);
...
#' @param method the distance measure to be used. Currently only `"euclidean"` is
#'  supported
