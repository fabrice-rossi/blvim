test_that("grid_var_autoplot works as expected", {
  ...
  vdiffr::expect_doppelganger(
    ...
    \() print(grid_var_autoplot(models_df, iterations, with_names = TRUE))
  )
  destination_names(models_df$sim) <- sample(letters, 15)
  vdiffr::expect_doppelganger(
    "Default flow matrices conv null names",
    \() print(grid_var_autoplot(models_df, converged, with_names = TRUE))
  )
  vdiffr::expect_doppelganger(
  ...
  vdiffr::expect_doppelganger(
    ...
    \() print(grid_var_autoplot(models_df, iterations, flow = "destination", with_names = TRUE))
  )
  destination_names(models_df$sim) <- NULL
  vdiffr::expect_doppelganger(
    "Default attr conv names",
    \() print(grid_var_autoplot(models_df, converged, flow = "attractiveness", with_names = TRUE))
  )
  vdiffr::expect_doppelganger(
    "Default destination iter pos",
    \() print(grid_var_autoplot(models_df, iterations, flow = "destination", with_positions = TRUE))
  )
  vdiffr::expect_doppelganger(
  ...
  vdiffr::expect_doppelganger(
    ...
    \() print(grid_var_autoplot(models_df, iterations,
      flow = "attractiveness", with_positions = TRUE, cut_off = 1, qmin = 0, qmax = 1
    ))
  )
  vdiffr::expect_doppelganger(
...
test_that("grid_var_autoplot errors and warnings are triggered", {
  ...
  expect_warning(grid_var_autoplot(models_df, iterations, with_positions = TRUE))
