...
#| fig.width: 8.5
#| fig.align: center
grid_autoplot(models_center_df, flows = "full", with_positions = TRUE) +
...
cluster 1:
```{r}
autoplot(euro_models_center[[1]], flows="full", with_positions = TRUE,
  arrow(length = unit(0.01, "npc"))) +
...
Or cluster 5:
```{r}
autoplot(euro_models_center[[5]], flows="full", with_positions = TRUE,
  arrow(length = unit(0.01, "npc"))) +
  scale_linewidth_continuous(range = c(0, 2)) +
  coord_sf(default_crs = sf::st_crs(4326))
```
